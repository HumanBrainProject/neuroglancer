...
import {isNotFoundError, responseJson, parseSpecialUrl, cancellableFetchOk} from 'neuroglancer/util/http_request';
...
function getAttributes(
    chunkManager: ChunkManager, credentialsProvider: any,
    url: string): Promise<any> {
  return chunkManager.memoize.getUncounted(
      {type: 'zarr:.zattrs json', url, credentialsProvider: getObjectId(credentialsProvider)},
      async () => {
        try {
          const json = await cancellableFetchOk(url + '/.zattrs', {}, responseJson)
          verifyObject(json);
          return json;
        } catch (e) {
...
function getMetadata(
  ...
  return chunkManager.memoize.getUncounted(
      ...
      async () => {
        try {
          const json = await cancellableFetchOk(url + '/.zarray', {}, responseJson);
...
function validateSeparator(
  ...
  if (actualSeparator) return actualSeparator
  if (expectedSeparator) return expectedSeparator
  return '.'
...
export class ZarrDataSource extends DataSourceProvider {
  ...
  get(options: GetDataSourceOptions): Promise<DataSource> {
    ...
    return options.chunkManager.memoize.getUncounted(
        {'type': 'zarr:MultiscaleVolumeChunkSource', providerUrl, dimensionSeparator}, async () => {
          const url =
              parseSpecialUrl(providerUrl);
          const [metadata, attrs] = await Promise.all([
            getMetadata(options.chunkManager, null, url),
            getAttributes(options.chunkManager, null, url)
          ]);
          ...
          if (metadata === undefined) {
            ...
            multiscaleInfo = await resolveOmeMultiscale(
                options.chunkManager, null, dimensionSeparator, multiscale);
          } else {
          ...
          const volume = new MultiscaleVolumeChunkSource(
            options.chunkManager, null, multiscaleInfo);
          return {
  ...
  async completeUrl(options: CompleteUrlOptions) {
    ...
    return await completeHttpPath(
        options.providerUrl, options.cancellationToken);
  }
}
